...
function createTelemetryTool(state: MissionState, speaker: string, record: CouncilActivity) {
  return tool({
    ...
    parameters: z.object({
      system: z.string().describe("A named system, or 'all' for all readings."),
    }),
    async execute({ system }) {
      const readings =
        system.toLowerCase() === "all"
          ? state.telemetry
          : state.telemetry.filter((item) =>
              item.label.toLowerCase().includes(system.toLowerCase())
            );
      const resolved = readings.length ? readings : state.telemetry;
      const label =
        resolved.length === state.telemetry.length
          ? "the full telemetry set"
          : resolved.map((reading) => reading.label).join(" and ");
  ...
  return tool({
    ...
    async execute({ topic }) {
      record({
        speaker,
        kind: "evidence",
        message: "Reviewed the " + topic + " safety procedure.",
      });
      return [
  ...
  return tool({
    ...
    async execute({ source }) {
      ...
      record({
        speaker,
        kind: "evidence",
        message: "Cross-checked the " + source + " report: " + finding,
      });
      return finding;
    },
  });
...
function specialist(
  ...
  return new Agent({
    ...
    tools: [
      createTelemetryTool(state, name, record),
      createProtocolTool(state, name, record),
      createVerificationTool(state, name, record),
    ],
  });
}
