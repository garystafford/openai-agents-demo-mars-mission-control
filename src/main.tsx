...
const actionLabels: Record<string, string> = {
  ...
  shed_nonessential_load: "Shed nonessential load",
  isolate_scrubber: "Isolate scrubber loop",
  verify_orbital_weather: "Verify orbital weather",
  deploy_repair_drone: "Deploy repair drone",
  switch_to_backup_relay: "Switch to backup relay",
};
const missionTeam = [
  {
    name: "NOVA",
...
type Mission = {
  ...
  scenario: { title: string; briefing: string; activeRisks: string[] };
...
async function api<T>(path: string, method = "GET", body?: unknown): Promise<T> {
  ...
  if (!response.ok)
    throw new Error(
      (await response.json().catch(() => ({}))).error ?? "Mission control link failed."
    );
  ...
  async function conveneCouncil(request?: string, previousPlan?: DecisionPlan) {
    ...
    try {
      const response = await fetch("/api/mission/convene", {
        ...
        body: isReview ? JSON.stringify({ reviewRequest: request, previousPlan }) : undefined,
      });
      if (!response.ok || !response.body)
        throw new Error("The mission team link could not be opened.");
          ...
          setMission((current) =>
            current
              ? {
                  ...current,
                  reports: [
                    ...current.reports.filter((report) => report.agent !== event.report!.agent),
                    event.report!,
                  ],
                }
              : current
          );
        else if (event.type === "complete" && event.state && event.plan) {
          setPlan(event.plan);
    ...
    <main>
      ...
      <section className="hero">
        <div>
          ...
          <div className="countdown">{String(mission.minutesToImpact).padStart(2, "0")}:00</div>
          <p className="muted">until impact window</p>
        </div>
            ...
            <div className="modal-heading">
              <div>
                ...
                <h2 id="technical-overview-title">
                  How this mission demonstrates OpenAI Agents SDK
                </h2>
              </div>
              <button
              ...
              <div className="specialist-grid">
                {missionTeam.map((member) => (
                  <article className="team-member" key={member.name}>
                    ...
                    <small>
                      {member.role} · {profileLabel(member.name)}
                    </small>
                    <p>{member.focus}</p>
                  </article>
                ))}
      ...
      <section className="layout">
            ...
            {mission.phase === "resolved" ? (
              ...
              <>
                ...
                <div className="button-row">
                  <button
                    onClick={() => void act("approve", "/api/mission/approve", { approved: true })}
                    disabled={Boolean(busy)}
                  >
                    Authorize plan
                  </button>
                  <button
                    className="danger"
                    onClick={() => void act("decline", "/api/mission/approve", { approved: false })}
                    disabled={Boolean(busy)}
                  >
                    Decline
                  </button>
              ...
              <>
                <p className="eyebrow">
                  Plan underway · report {Math.min(mission.monitoringIntervals + 1, 3)} of 3
                </p>
                <h2>Monitor the deployed response</h2>
                <p className="muted">
                  Authorization dispatched the plan, but it takes effect over the simulated mission
                  clock. Advance one four-minute interval for the next changing field report.
                </p>
                <button
              ...
              <>
                ...
                <button onClick={() => void conveneCouncil()} disabled={Boolean(busy)}>
                  {busy === "council"
                    ? "Mission team is assessing…"
                    : "Get mission team assessment"}
                </button>
              </>
            ) : reviewMode ? (
              <>
                <p className="eyebrow">Interactive plan review</p>
                <h2>Challenge or refine the proposal</h2>
                <p className="muted">
                  Ask about evidence, trade-offs, or a safer alternative. The Director will continue
                  this mission’s investigation before you choose whether to submit the plan.
                </p>
                <div className="review-presets">
                  <button
                    ...
                    onClick={() =>
                      setReviewRequest(
                        "Challenge the key trade-off and identify the most likely plan failure."
                      )
                    }
                    disabled={Boolean(busy)}
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setReviewRequest(
                        "What additional verification would most improve confidence before authorization?"
                      )
                    }
                    disabled={Boolean(busy)}
                  >
                ...
                <label className="review-question">
                  <span>Question for the Mission Director</span>
                  <textarea
                    value={reviewRequest}
                    onChange={(event) => setReviewRequest(event.target.value)}
                    placeholder="For example: Is there a safer alternative if the orbital forecast is wrong?"
                    disabled={Boolean(busy)}
                  />
                </label>
                <div className="button-row">
        ...
        <div className="agent-column">
          {(busy === "council" || visibleCouncilLog.length > 0) && (
            <section
              className={"panel council-record" + (busy === "council" ? " panel-active" : "")}
              aria-busy={busy === "council"}
            >
              <p className="eyebrow">
                {busy === "council" ? "Team assessment in progress" : "Team assessment complete"}
              </p>
              <h2>Team activity</h2>
        ...
        <aside className="operations-column">
          <InteractionMap
            ...
            plan={busy === "council" ? null : activePlan}
            running={busy === "council"}
            onShowTeam={() => setShowTeam(true)}
          />
          <section
            className={"panel assessments-panel" + (busy === "council" ? " panel-active" : "")}
            aria-busy={busy === "council"}
          >
                  ...
                  <article className="report" key={report.agent}>
                    <div className="report-title">
                      <div>
                        ...
                        <small>
                          {report.role} · {profileLabel(report.agent)}
                        </small>
                      </div>
