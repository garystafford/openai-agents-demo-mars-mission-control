...
function InteractionMap({ reports, entries, plan, running, onShowTeam }: { reports: Report[]; entries: CouncilEntry[]; plan: DecisionPlan | null; running: boolean; onShowTeam: () => void }) {
  ...
  return (
    <section className={"panel interaction-map" + (running ? " panel-active" : "")} aria-label="Agent interaction flow" aria-busy={running}>
...
function App() {
  ...
  return <main>
    ...
    {activePlan && <section className="panel brief"><p className="eyebrow">Director’s recommendation</p><h2>{activePlan.headline}</h2><div className="plan-actions">{activePlan.actions.map((action) => <span key={action}>{actionLabels[action] ?? action}</span>)}</div><p>{activePlan.rationale}</p>{activePlan.uncertainties.length > 0 && <p className="uncertainties"><strong>Still uncertain:</strong> {activePlan.uncertainties.join(" · ")}</p>}</section>}</div>
      <div className="agent-column">{(busy === "council" || visibleCouncilLog.length > 0) && <section className={"panel council-record" + (busy === "council" ? " panel-active" : "")} aria-busy={busy === "council"}><p className="eyebrow">{busy === "council" ? "Team assessment in progress" : "Team assessment complete"}</p><h2>Team activity</h2>{visibleCouncilLog.length === 0 ? <p className="muted">The Director is opening the mission channel…</p> : <ol>{visibleCouncilLog.map((entry) => <li key={entry.id} className={entry.kind}><span className="activity-speaker"><AgentMark name={entry.speaker} />{entry.speaker}</span><p>{entry.message}</p></li>)}</ol>}{busy === "council" && <p className="muted">New entries appear as the Director selects evidence and specialists.</p>}</section>}<section className="panel timeline"><p className="eyebrow">Mission record</p><h2>Mission events</h2><ol>{mission.timeline.slice().reverse().map((event, index) => <li key={event.time + index}><time>{event.time}</time><span className={event.kind} />{event.event}</li>)}</ol></section></div>
      <aside className="operations-column"><InteractionMap reports={mission.reports} entries={visibleCouncilLog} plan={busy === "council" ? null : activePlan} running={busy === "council"} onShowTeam={() => setShowTeam(true)} /><section className={"panel assessments-panel" + (busy === "council" ? " panel-active" : "")} aria-busy={busy === "council"}><div className="assessment-heading"><div><p className="eyebrow">Team output</p><h3>Specialist assessments</h3></div><span>{mission.reports.length}/4 consulted</span></div>{mission.reports.length === 0 ? <div className="empty-state">{busy === "council" ? "The Director is selecting the first specialist and their evidence sources." : "The specialists are ready. The Director will bring in only those whose domain can reduce this incident’s uncertainty."}</div> : <div className="reports">{mission.reports.map((report) => <article className="report" key={report.agent}><div className="report-title"><div><span className="agent-name"><AgentMark name={report.agent} />{report.agent}</span><small>{report.role} · {profileLabel(report.agent)}</small></div><Badge status={report.status} /></div><p>{report.recommendation}</p><div className="evidence">{report.evidence.map((item) => <span key={item}>{item}</span>)}</div><footer><span>Confidence {Math.round(report.confidence * 100)}%</span><span>Trade-off: {report.tradeoff}</span></footer></article>)}</div>}</section></aside>
    </section>
  </main>;
}
